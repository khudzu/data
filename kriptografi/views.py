...
def loginView(request):
    ...
    if request.method == 'GET':
        ...
        if user is not None:
            ...
            messages.add_message(request, messages.ERROR, 'Username atau password anda salah, silahkan masukkan dengan benar!')
...
def logoutView(request):
    ...
    if request.method == "POST":
        if request.POST["logout"]=="Iya, Saya ingin logout":
            if ssim.objects.filter(username=username_login).exists() and crypto.objects.filter(username=username_login).exists():
...
def registerView(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(username=username_register).exists():
            messages.add_message(request, messages.ERROR, 'Username telah digunakan, silahkan menggunakan Username yang lain!')
            return redirect('register')
        elif User.objects.filter(email=email_register).exists():
            messages.add_message(request, messages.ERROR, 'Email telah digunakan, silahkan menggunakan Email yang lain!')
            return redirect('register')
        else:
